 ...
 **/
@DefaultService
public class JvmMemoryService implements CommonService {

    private static final Logger logger = LoggerFactory.getLogger(JvmMemoryService.class);
    ...
    @Override
    public void start() {
        sendMetricFuture = new ScheduledThreadPoolExecutor(1, new DefaultThreadFactory("JvmMemoryService-"))
                .scheduleAtFixedRate(new RunnableWithExceptionProtection(sender, th -> {
            logger.error("JvmMemoryService collect and upload failure.", th);
        ...
        public void run() {
            if (status == ChannelStatus.CONNECTION) {
                try {
                    ...
                    JVMMemoryMetric.Builder builder = MemoryProvider.INSTANCE.getJVMMemory();
                    ...
                    message.setCode(CommandCode.JVM_MEMORY.getCode());
                    message.setBody("jvm memory");
                    message.setId("0000");
                    ...
                    client.getChannel().writeAndFlush(message.build()).addListener((ChannelFutureListener) future -> {
                        if (future.isSuccess()) {
                            logger.debug("JvmMemoryService send successful");
                        } else {
                            logger.error("JvmMemoryService send {} fail", AgentConfig.Agent.INSTANCE_UUID);
                        }
                    });
                } catch (Throwable t) {
                    logger.error("send JVM memory metrics to Collector fail.", t);
                }
            }
        }
