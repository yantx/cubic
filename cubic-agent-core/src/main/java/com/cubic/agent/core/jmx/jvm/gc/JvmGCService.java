...
import com.cubic.serialization.agent.v1.JVMGCMetricOrBuilder;
 ...
 **/
@DefaultService
public class JvmGCService implements CommonService {

    private static final Logger logger = LoggerFactory.getLogger(JvmGCService.class);
    ...
    @Override
    public void start() {
        sendMetricFuture = new ScheduledThreadPoolExecutor(1, new DefaultThreadFactory("JvmGCService-"))
                .scheduleAtFixedRate(new RunnableWithExceptionProtection(sender, th -> {
            logger.error("JvmGCService collect and upload failure.", th);
        ...
        public void run() {
            if (status == ChannelStatus.CONNECTION) {
                try {
                    ...
                    List<JVMGCMetric.Builder> builders = GCProvider.INSTANCE.getJVMGCMetrics();
                    for (JVMGCMetric.Builder builder : builders) {
                        ...
                        message.setCode(CommandCode.JVM_GC.getCode());
                        message.setBody("jvm gc");
                        message.setId("0000");
                        ...
                        client.getChannel().writeAndFlush(message.build()).addListener((ChannelFutureListener) future -> {
                            if (future.isSuccess()) {
                                logger.debug("JvmGCService send successful");
                            } else {
                                logger.error("JvmGCService send {} fail", AgentConfig.Agent.INSTANCE_UUID);
                            }
                        });
                    }
                } catch (Throwable t) {
                    logger.error("send JVM gc metrics to Collector fail.", t);
                }
            }
        }
